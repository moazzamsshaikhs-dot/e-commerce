<?php
// action/settings/upload-banner.php
session_start();
require_once dirname(__DIR__, 4) . '/includes/config.php';
require_once dirname(__DIR__, 4) . '/includes/auth-check.php';

header('Content-Type: application/json');

error_log("=== Upload Banner Started ===");
...
function uploadVendorImage($file, $type = 'banner', $vendor_id) {
...
try {
    ...
    $banner = uploadVendorImage($_FILES['store_banner'], 'banner', $vendor_id);
    error_log("Banner uploaded: $banner");
    ...
    if ($exists) {
        // Get old banner to delete
        $stmt = $db->prepare("SELECT store_banner FROM vendor_settings WHERE vendor_id = ?");
        $stmt->execute([$vendor_id]);
        $old_banner = $stmt->fetchColumn();
        
        // Update
        $stmt = $db->prepare("UPDATE vendor_settings SET store_banner = ?, updated_at = NOW() WHERE vendor_id = ?");
        $stmt->execute([$banner, $vendor_id]);
        
        // Delete old banner file if exists
        if ($old_banner && $old_banner !== $banner) {
            $old_path = $_SERVER['DOCUMENT_ROOT'] . SITE_URL .'/uploads/vendors/' . $old_banner;
        ...
        // Insert
        $stmt = $db->prepare("INSERT INTO vendor_settings (vendor_id, store_banner, created_at, updated_at) VALUES (?, ?, NOW(), NOW())");
        $stmt->execute([$vendor_id, $banner]);
    }
    ...
    try {
        $log = $db->prepare("INSERT INTO user_activities (user_id, activity_type, description, ip_address, user_agent, created_at) VALUES (?, 'upload_banner', ?, ?, ?, NOW())");
        $log->execute([$vendor_id, "Uploaded new store banner", $ip, $ua]);
    ...
    error_log("Error in upload-banner.php: " . $e->getMessage());
