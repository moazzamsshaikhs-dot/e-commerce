...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $db->prepare("SELECT id, key_name FROM vendor_api_keys WHERE id = ? AND vendor_id = ?");
        ...
        $new_api_key = 'sk_live_' . bin2hex(random_bytes(24));
        $new_api_secret = 'sk_' . bin2hex(random_bytes(32));
        ...
        ");
        $stmt->execute([$new_api_key, $new_api_secret, $key_id, $vendor_id]);
        
        // Log the regeneration
        logVendorActivity($vendor_id, 'regenerate_api_key', "Regenerated API key #$key_id - {$key_data['key_name']}");
