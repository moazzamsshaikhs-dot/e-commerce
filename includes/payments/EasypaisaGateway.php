...
class EasypaisaGateway extends PaymentGateway implements PaymentGatewayInterface {
    
    protected $gatewayCode = 'easypaisa';
    protected $gatewayName = 'Easypaisa';
    ...
    public function processCustomerPayment($order, $paymentData) {
        try {
            ...
            if (empty($mobileNumber)) {
                return [
                    'success' => false,
                    'message' => 'Mobile number is required for Easypaisa payment'
                ];
            }
            ...
            return [
                ...
                'message' => 'Easypaisa payment initiated. Please check your phone for OTP.',
                'redirect' => SITE_URL . 'user/orders/order-confirmation.php?id=' . $order['id']
            ];
            
        } catch (\Exception $e) {
            error_log("Easypaisa Error: " . $e->getMessage());
            return [
                'success' => false,
                'message' => 'Easypaisa payment failed: ' . $e->getMessage()
            ];
        }
    }
    ...
    public function processVendorPayout($vendorId, $amount, $payoutData) {
        try {
            // Get vendor's Easypaisa account
            $stmt = $this->db->prepare("
                ...
                WHERE vma.vendor_id = ? AND vma.account_type = 'easypaisa' AND vma.is_default = 1
                LIMIT 1
            ");
            ...
            if (!$mobileAccount) {
                return [
                    'success' => false,
                    'message' => 'Vendor has no Easypaisa account configured'
                ];
            }
            ...
            return [
                'success' => true,
                'transaction_id' => $transactionId,
                'message' => 'Easypaisa payout initiated'
            ];
            
        } catch (\Exception $e) {
            error_log("Easypaisa Payout Error: " . $e->getMessage());
            return [
                'success' => false,
    ...
    public function refundPayment($transactionId, $amount = null) {
        return [
            'success' => false, 
            'message' => 'Refund not implemented for Easypaisa'
        ];
    }
    ...
    public function getPaymentForm($order, $userAccounts = []) {
        ...
        $html = '<div class="easypaisa-form">';
        
        $adminAccounts = $this->getAdminEasypaisaAccounts();
        
        if (!empty($adminAccounts)) {
            $html .= '<div class="alert alert-info mb-3">';
            $html .= '<i class="fas fa-info-circle me-2"></i>';
            $html .= 'Send payment to Easypaisa account: <strong>' . 
                     htmlspecialchars($adminAccounts[0]['account_name'] ?? 'Easypaisa Account') . '</strong>';
        ...
        $html .= '<small class="text-muted">Enter the mobile number linked to your Easypaisa account</small>';
        $html .= '</div>';
        
        $html .= '<div class="mb-3">';
        $html .= '<label class="form-label">CNIC (Optional)</label>';
        $html .= '<input type="text" name="cnic" class="form-control" placeholder="XXXXX-XXXXXXX-X">';
        $html .= '</div>';
        ...
        $html .= '1. Open Easypaisa app on your phone<br>';
        $html .= '2. Go to "Send Money" or "Pay"<br>';
        ...
        $html .= '<input type="hidden" name="payment_method" value="easypaisa">';
        $html .= '</div>';
        
        return $html;
    }
    
    private function getAdminEasypaisaAccounts() {
        try {
            $stmt = $this->db->prepare("
            ...
            error_log("Error getting admin Easypaisa accounts: " . $e->getMessage());
            return [];
        }
    }
}